...
function result_validate_email($email){
    $sql="SELECT * FROM taikhoan WHERE email='".$email."'";
    $result = pdo_query($sql);
    return $result;
}
function update_account_id($id,$user,$email,$add,$sdt,$target_file){
    $sql = "UPDATE taikhoan SET user='".$user."',email='".$email."',sdt='".$sdt."',img='".$target_file."',address='".$add."' WHERE id=".$id;
    pdo_execute($sql);
}
