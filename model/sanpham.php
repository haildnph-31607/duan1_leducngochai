...
function select_bienthe($idsp){
    $sql="SELECT sp.name AS namesp ,
    sp.img AS img ,
    ss.idsp AS idsp ,
     ss.price AS price ,
      ss.quantity AS quantity FROM sanpham_shared ss INNER JOIN sanpham sp ON sp.id = ss.idsp WHERE idsp= '".$idsp."'";
...
function sp_banchay(){
    ...
    sp.luotmua 
       ...
       WHERE ss.idsize = 3 ORDER BY sp.luotmua DESC LIMIT 16";
       $result = pdo_query($sql);
       return $result;
}
function select_bl_sp($idsp){
    $sql="SELECT * FROM sanpham WHERE id=".$idsp;
    $result = pdo_query_one($sql);
    return $result;
}
function soluonghh_ad(){
    $sql="SELECT quantity FROM sanpham_shared ";
    $result = pdo_query($sql);
    return $result;
}
// function update_sanpham($file_img,$file_img1,$file_img2,$file_img3,$id,$name,$target_file,$target_file1,$target_file2,$target_file3,$color,$color1,$color2,$mota,$dateup,$iddm){
// if($file_img !="" || $file_img1 !="" || $file_img2 !="" || $file_img3 !=""){
...
// }else{
//     $sql="UPDATE `sanpham` SET `name`='$name',`color`='$color',`color1`='$color1',`color2`='$color2',`mota`='$mota',`dateup`='$dateup',`iddm`='$iddm' WHERE id=".$id;
// }
//     // var_dump($sql);die();
//     pdo_execute($sql);
// }
